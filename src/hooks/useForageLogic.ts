...
export function useForageLogic() {
  ...
  const determineRarityAndSuccess = (totalRoll: number) => {
    const rollRanges = [
      ...
      { min: 31, max: Infinity, rarity: 'unico' as const, isNative: false, dc: 31, uniqueChance: 0.1 }
    ];
    ...
    for (const range of rollRanges) {
      if (totalRoll >= range.min && totalRoll <= range.max) {
        ...
        return {
          rarity: finalRarity,
          success: true,
          dcResult: { dc: range.dc, range: `${range.min}-${range.max === Infinity ? '+' : range.max}` },
