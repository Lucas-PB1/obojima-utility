...
class FirebaseStorageService {
  ...
  subscribeToCollectedIngredients(callback: (ingredients: CollectedIngredient[]) => void): () => void {
    ...
    try {
      ...
      this.collectedIngredientsUnsubscribe = onSnapshot(q, (snapshot) => {
  ...
  async updateCollectedIngredient(id: string, updates: Partial<CollectedIngredient>): Promise<void> {
    if (!this.isClient() || !this.getUserId()) return;
  ...
  subscribeToForageAttempts(callback: (attempts: ForageAttempt[]) => void): () => void {
    ...
    try {
      ...
      this.forageAttemptsUnsubscribe = onSnapshot(q, (snapshot) => {
