...
export function AdminUserHistoryTab({ data }: AdminUserHistoryTabProps) {
  ...
  const columns: Column<ForageAttempt>[] = [
    ...
    },
    {
      ...
      render: (_, item) => <span className="capitalize text-sm text-totoro-gray">{item.region}</span>
    },
    {
      key: 'ingredient',
    ...
    },
    {
      ...
      render: (_, item) => <span className="font-mono font-bold text-totoro-blue bg-totoro-blue/10 px-2 py-0.5 rounded text-xs">{item.roll}</span>
    },
    {
      key: 'success',
  ...
  const mobileRenderer = (attempt: ForageAttempt) => {
    ...
    const englishName = attempt.ingredient ? getEnglishName(attempt.ingredient.id, attempt.ingredient.raridade) : null;
      ...
      <div className="flex items-center justify-between p-4 bg-white/50 rounded-2xl border border-white shadow-sm mb-3">
        <div className="flex items-center gap-4">
           <div className={`w-12 h-12 rounded-[18px] flex items-center justify-center text-2xl shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-totoro-blue/5 ${
                attempt.success ? 'bg-totoro-green/10 text-totoro-green' : 'bg-totoro-orange/10 text-totoro-orange'
           }`}>
             {attempt.success ? '✅' : '❌'}
           </div>
           <div className="flex flex-col gap-0.5">
              ...
              {localized && (
                <div className="mt-1 flex flex-col">
                   ...
                   <span className="text-[9px] text-totoro-blue/60 italic capitalize">{englishName}</span>
                </div>
              )}
           </div>
        </div>
        ...
        <div className="flex flex-col items-end gap-1">
             ...
             <span className="text-xl font-black text-totoro-blue">
                {attempt.roll}
             </span>
        </div>
      </div>
