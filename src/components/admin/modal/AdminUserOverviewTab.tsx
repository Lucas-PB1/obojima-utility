    ...
    <div className="space-y-6">
      <div className="glass-panel p-6">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-8">
          <div className="w-24 h-24 rounded-full bg-totoro-blue/10 flex items-center justify-center text-4xl border-4 border-white shadow-lg shrink-0">
            {user.photoURL ? (
              <img
                src={user.photoURL}
          ...
          <div className="flex-1 text-center md:text-left space-y-2 w-full">
            ...
            <div className="flex items-center justify-center md:justify-start gap-2 flex-wrap">
              ...
              <span
                className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                  !user.disabled ? 'bg-totoro-green/10 text-totoro-green' : 'bg-totoro-orange/10 text-totoro-orange'
                }`}
              >
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          ...
          >
            ...
            <span className="text-[10px] font-black uppercase tracking-wider">{t('admin.users.action.edit')}</span>
          </button>
          <button
          ...
          >
            <span className="text-xl group-hover:scale-110 transition-transform">{!user.disabled ? '🚫' : '✅'}</span>
          ...
          >
            <span className="text-xl group-hover:scale-110 transition-transform">🗑️</span>
            <span className="text-[10px] font-black uppercase tracking-wider">{t('admin.users.action.delete')}</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 p-4 bg-totoro-gray/5 rounded-2xl border border-totoro-gray/5">
           <div>
             <label className="text-[10px] font-black uppercase tracking-wider text-totoro-gray/40 mb-1 block">UID</label>
      ...
      <div className="glass-panel p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-3xl font-black text-totoro-green mb-1">{stats.ingredientsCount}</div>
            <div className="text-[10px] font-bold text-totoro-gray/40 uppercase tracking-widest">{t('admin.modal.tabs.inventory')}</div>
          </div>
          <div className="text-center">
             <div className="text-3xl font-black text-totoro-blue mb-1">{stats.potionsCount}</div>
             <div className="text-[10px] font-bold text-totoro-gray/40 uppercase tracking-widest">{t('admin.modal.tabs.potions')}</div>
          </div>
          <div className="text-center">
             <div className="text-3xl font-black text-totoro-orange mb-1">{stats.attemptsCount}</div>
             <div className="text-[10px] font-bold text-totoro-gray/40 uppercase tracking-widest">{t('admin.modal.tabs.history')}</div>
          </div>
        </div>
      </div>
    </div>
